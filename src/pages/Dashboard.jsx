...
const Dashboard = ({ user, pets, reminders, upcomingReminders, completedRemindersCount, setView, toggleReminder }) => {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-brand-600 to-brand-800 rounded-3xl p-8 text-white shadow-xl shadow-brand-900/10 col-span-1 md:col-span-2 flex flex-col justify-between min-h-[200px]">
          ...
          <div className="relative z-10 flex gap-3 mt-6">
            <button onClick={() => setView(ViewState.MY_PETS)} className="bg-white text-brand-700 hover:bg-brand-50 transition px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm">Manage Pets</button>
            <button onClick={() => setView(ViewState.COMMUNITY)} className="bg-brand-700/50 hover:bg-brand-700/70 text-white backdrop-blur-md transition px-5 py-2.5 rounded-xl text-sm font-bold">Community</button>
          </div>
        </div>
