...
const PetDetail = ({ pet, reminders, onBack, onUpdatePet, onDeletePet, onAddReminder, onToggleReminder, onDeleteReminder }) => {
  // 1. Define all hooks at the top level (Always execute hooks in the same order)
  const [isAddingReminder, setIsAddingReminder] = useState(false);
  const [isEditingPet, setIsEditingPet] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  
  // Initialize form state safely (even if pet is null initially)
  ...
  // 2. Conditional rendering check happens AFTER hooks
  if (!pet) {
    return (
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-brand-600 transition font-medium">
          ...
          {activeReminders.length === 0 ? (
            ...
            activeReminders.map(r => (
              <div key={r.id} className="flex items-center justify-between p-5 hover:bg-slate-50 rounded-2xl border border-slate-100 transition group bg-white">
                 <div className="flex items-center gap-4">
                    <button onClick={() => onToggleReminder(r.id)} className="w-6 h-6 rounded-full border-2 border-slate-300 hover:border-brand-500 flex items-center justify-center transition-all" title="Mark as completed">
                    </button>
                    <div>
                 ...
                 <button onClick={() => onDeleteReminder(r.id)} className="text-slate-300 hover:text-rose-500 p-2 opacity-0 group-hover:opacity-100 transition" title="Delete">
                 ...
                 completedReminders.map(r => (
                   <div key={r.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 opacity-75 hover:opacity-100 transition">
                     ...
                     <div className="flex gap-2">
                       <button onClick={() => onToggleReminder(r.id)} className="text-slate-400 hover:text-brand-600 p-1.5 hover:bg-white rounded-lg transition" title="Undo Completion">
                          <RotateCcw className="w-4 h-4" />
                       </button>
                       <button onClick={() => onDeleteReminder(r.id)} className="text-slate-400 hover:text-rose-600 p-1.5 hover:bg-white rounded-lg transition" title="Delete Permanently">
